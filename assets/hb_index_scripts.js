...
const randomNotices = [
    ...
    { 
        ...
        content: "서버 트래픽 과부하를 막고 모바일 환경 최적화를 위해, 앞으로 효빈위키에 업로드되는 고용량 PNG/JPG 파일은 시스템에서 자동 압축 및 WebP 포맷으로 변환되어 서빙됩니다. 단, 위키 업로드 시 파일명 자체는 여전히 '한국어_단어.webp' 규칙을 지켜주셔야 시스템에서 정상적으로 파싱됩니다." 
    },
    { 
    ...
    },
    {
        ...
        content: "도움말 규정에 분명히 명시했음에도 불구하고 여전히 파일명을 'bus_image01.jpg' 같이 성의 없이 올리는 유저들이 빈몰하고 있습니다. 효빈위키 시스템 파싱 특성상 이미지는 반드시 '한국어 단어 이름.webp' 형식이어야만 정상 출력됩니다. 계속 위반 시 업로드 권한을 박탈합니다."
    },
    {
    ...
    },
    {
        ...
        content: "4호선(주황색, #FF5522) 다로나의 공식 디자인에 뱅드림 토야마 카스미 특유의 뿔 머리나 과장된 고양이 귀 별 모양을 억지로 합성하여 업로드한 불법 일러스트 이미지가 갤러리에서 모두 일괄 삭제되고, 효빈도시철도에서 배포한 공식 일러스트 원본(.webp)으로 전면 교체되었습니다."
    },
    {
    ...
    },
    {
        ...
        content: "효빈시를 벗어나 인근 위성도시를 잇는 빈효선 광역전철(연청색, #6677CC)의 소심한 마스코트 전노아의 최상단 프로필 사진이 고화질 원본 파일(한국어_단어.webp)로 교체되었습니다. 뱅드림 쿠라타 마시로 이미지를 교묘하게 덧그린 저화질 훼손판 이미지를 재업로드 시 즉시 계정 차단됩니다."
    },
    {
    ...
    {
        id: "r90", title: "[공지] 문서 내 삽입되는 모든 지역 사진은 시스템 오류 방지를 위해 '한국어 단어.webp' 로 통일",
        content: "마지막으로 다시 한번 강력히 공지합니다. 효빈시 관내 지하철역, 버스 정류장, 풍경 사진, 주요 랜드마크 등을 효빈위키 파일 서버에 업로드할 때는 무조건 '한국어 단어.webp' 명명 규칙을 철저히 준수해야 합니다. 영문명이나 jpg 포맷 업로드 시 엑스박스 등 시스템 파싱 오류를 유발하므로 관리자 권한으로 즉시 삭제 처리됩니다."
    }
];
