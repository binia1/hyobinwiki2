...
// =====================================================================
// 🚀 [효빈위키 최종 길이 우선 포함 매칭 패치] : '김시연다' -> '김시연' 완벽 직행
// =====================================================================
(function() {
    'use strict';

    // --- [1파트] 넘어옴 배너 (?from=...) 렌더링 로직 ---
    function initRedirectBanner() {
        try {
            ...
            document.querySelectorAll('#wiki-redirect-banner, .wiki-redirect-banner').forEach(b => b.remove());

            const bannerDiv = document.createElement('div');
            bannerDiv.id = 'wiki-redirect-banner';
            bannerDiv.className = 'wiki-redirect-banner';
            bannerDiv.style.cssText = `
                border: 1px solid #b8d4e9 !important;
            ...
            console.error("[RedirectBanner Error]", err);
        }
    }
...
// --- [2파트] 안전한 이동 헬퍼 ---
    ...
    function executeSearch(query) {
        ...
        if (/^\d+$/.test(query)) {
            ...
            var id = (n >= 1 && n <= 9) ? "0" + n : "" + n;
            redirectToWithFrom("bus_" + id + ".html", query);
            return;
        }

        // 1순위: 별칭(Aliases) 완벽 일치 체크
        if (window.HB_WIKI_ALIASES) {
            if (window.HB_WIKI_ALIASES[query]) {
                redirectToWithFrom(window.HB_WIKI_ALIASES[query], query);
                return;
            }
            var cleanQ = query.replace(/역$/, '');
            if (window.HB_WIKI_ALIASES[cleanQ]) {
                redirectToWithFrom(window.HB_WIKI_ALIASES[cleanQ], query);
                return;
            }
        }

        // 2순위: 문서 목록에서 포함 관계 매칭 (글자수 긴 것 우선 정렬로 오탐지 원천 차단)
        if (window.HB_WIKI_PAGES && window.HB_WIKI_PAGES.length > 0) {
            // 가. 완전 일치 우선 확인
            var exactMatch = window.HB_WIKI_PAGES.find(function(p) {
                var title = typeof p === 'string' ? p : (p.title || '');
                return title === query || title === query + '역';
            });
            if (exactMatch) {
                var href = typeof exactMatch === 'string' ? exactMatch + '.html' : (exactMatch.href || exactMatch.title + '.html');
                redirectToWithFrom(href, query);
                return;
            }

            // 나. 포함 관계 매칭 (검색어가 문서 제목을 포함하거나, 문서 제목이 검색어를 포함하는 경우)
            var matchedPages = window.HB_WIKI_PAGES.filter(function(p) {
                var title = typeof p === 'string' ? p : (p.title || '');
                if (!title || title.length <= 1) return false; // 1글자짜리 단독 문서("시" 같은 것)는 단독 오탐지 방지를 위해 제외
                return query.indexOf(title) > -1 || title.indexOf(query) > -1;
            });

            if (matchedPages.length > 0) {
                // 핵심: 문서 제목 글자 수가 긴 것부터 내림차순 정렬 ("김시연" > "시")
                matchedPages.sort(function(a, b) {
                    var tA = typeof a === 'string' ? a : (a.title || '');
                    var tB = typeof b === 'string' ? b : (b.title || '');
                    return tB.length - tA.length;
                });

                var bestMatch = matchedPages[0];
                var href = typeof bestMatch === 'string' ? bestMatch + '.html' : (bestMatch.href || bestMatch.title + '.html');
                redirectToWithFrom(href, query);
                return;
            }
        }

        // 3순위: 매칭되는 게 없으면 안전하게 검색 결과 페이지로 이동
        location.href = "효빈위키 검색.html?q=" + encodeURIComponent(query);
    }
    ...
    document.addEventListener('submit', function(e) {
        var form = e.target;
        var input = form.querySelector('input[type="text"], input[name="q"], input#searchInput, input#headerSearchInput');
        if (input) {
            e.preventDefault();
    ...
    document.addEventListener('keydown', function(e) {
        if (e.key === 'Enter') {
            ...
            if (activeEl && activeEl.tagName === 'INPUT' && (activeEl.id.includes('Search') || activeEl.name === 'q' || activeEl.classList.contains('search') || activeEl.type === 'text')) {
                e.preventDefault();
                executeSearch(activeEl.value.trim());
            }
        }
    });
