...
# 1. 설정
WIKI_PATH = './'
RESULT_FILE = '나무위키_인덱스.html'

# 초성 리스트
CHOSUNG = ['ㄱ', 'ㄲ', 'ㄴ', 'ㄷ', 'ㄸ', 'ㄹ', 'ㅁ', 'ㅂ', 'ㅃ', 'ㅅ', 'ㅆ', 'ㅇ', 'ㅈ', 'ㅉ', 'ㅊ', 'ㅋ', 'ㅌ', 'ㅍ', 'ㅎ']

# 동적 항목 리스트 (시장님이 주신 데이터)
DYNAMIC_LIST = [
...
def get_chosung(text):
    """한글 초성을 추출합니다. 쌍자음까지 완벽하게 처리합니다."""
    if not text: return "#"
    
    # 공백이나 특수문자로 시작할 경우 처리
    first_char = text.strip()[0] 
    
    if '가' <= first_char <= '힣':
        # 한글 유니코드 공식: (글자 코드 - 44032) // 588 = 초성 인덱스
        idx = (ord(first_char) - 44032) // 588
        return CHOSUNG[idx]
    
    # 한글이 아닐 경우 (영어, 숫자 등) 대문자로 반환
    return first_char.upper() if first_char.isalnum() else "#"
...
# 3. 메인 로직
def generate_namu_style():
    category_map = {}
    sub_categories = {}

    print(f"🔍 나무위키 스타일 인덱스 생성 중...")

    # [A] 일반 파일 수집
    for filename in os.listdir(WIKI_PATH):
        if filename.endswith('.html') and filename != RESULT_FILE:
            try:
                    ...
                    if cat_box:
                        ...
                        for cat_full in cat_list:
                            ...
                            if '/' in cat_full:
                                ...
                                if parent not in sub_categories: sub_categories[parent] = set()
                                sub_categories[parent].add(cat_full)
...
<meta charset="UTF-8">
<style>
    body { font-family: sans-serif; background: #fff; padding: 20px; color: #373a3c; }
    h1 { font-size: 30px; border-bottom: 2px solid #373ad3; padding-bottom: 10px; }
    .cat-container { max-width: 1100px; margin: 0 auto; border: 1px solid #ccc; padding: 20px; }
    
    /* 하위 분류 박스 */
    .sub-box { background: #f9f9f9; border: 1px solid #ccc; padding: 15px; margin-bottom: 30px; }
    .sub-box h2 { font-size: 18px; margin-top: 0; color: #555; }
    .sub-list { display: grid; grid-template-columns: repeat(4, 1fr); gap: 10px; list-style: none; padding: 0; }
    .sub-list li::before { content: "• "; color: #373ad3; }

    /* 초성 섹션 */
    .chosung-group { margin-top: 40px; }
    .chosung-title { font-size: 22px; font-weight: bold; border-bottom: 1px solid #eee; margin-bottom: 15px; color: #373ad3; }
    .doc-grid { display: grid; grid-template-columns: repeat(3, 1fr); gap: 8px; list-style: none; padding: 0; }
    
    a { text-decoration: none; color: #0275d8; }
    a:hover { text-decoration: underline; color: #ba0000; }
    .tag { font-size: 11px; color: #aaa; margin-left: 5px; }
...
''')
        ...
        for cat in sorted(category_map.keys()):
            out.write(f'<h1 id="{cat}">분류: {cat}</h1>\n')

            # [1] 하위 분류 (있다면)
            if cat in sub_categories:
                out.write('<div class="sub-box"><h2>하위 분류</h2>\n<ul class="sub-list">\n')
                for sub in sorted(sub_categories[cat]):
                    sub_name = sub.split('/')[-1]
                    out.write(f'<li><a href="#{sub}">{sub_name}</a></li>\n')
                out.write('</ul></div>\n')

            # [2] 문서 목록 (초성별)
            docs = category_map[cat]
            grouped = {}
            for d in docs:
                cho = get_chosung(d['title'])
                if cho not in grouped: grouped[cho] = []
                grouped[cho].append(d)

            for cho in sorted(grouped.keys()):
                out.write(f'<div class="chosung-group">\n<div class="chosung-title">{cho}</div>\n<ul class="doc-grid">\n')
                for d in sorted(grouped[cho], key=lambda x: x['title']):
                    tag = '<span class="tag">[동적]</span>' if '?' in d['url'] or '#' in d['url'] else ""
                    out.write(f'<li><a href="{d["url"]}">{d["title"]}</a>{tag}</li>\n')
                out.write('</ul></div>\n')
    ...
    print(f"✅ 테스트용 인덱스 생성 완료! '{RESULT_FILE}' 파일을 확인하세요.")
