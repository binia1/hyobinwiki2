/**
 * 파일명: js/역대단체장_치원.js
 * 설명: 덕빈북도 치원군수 내비게이션 자동 생성
 */
document.addEventListener("DOMContentLoaded", function() {
    var containers = document.querySelectorAll(".hb-history-치원-nav");
...
<tr>
<td class="history-nav-toggle" colspan="5" onclick="toggleHistoryNav_치원()">
<span id="history-nav-text-치원">[ 펼치기 · 접기 ]</span>
</td>
</tr>
</thead>
<tbody id="history-nav-body-치원">
<tr>
...
// 개별 토글 함수 자동 생성
if (typeof window.toggleHistoryNav_치원 === 'undefined') {
    window.toggleHistoryNav_치원 = function() {
        var body = document.getElementById("history-nav-body-치원");
        var text = document.getElementById("history-nav-text-치원");
        if (!body || !text) return;
